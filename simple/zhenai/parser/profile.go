...
const (
	...
	mateRe     = `<div data-v-8b1eac0c="" class="m-btn">([^<]+)</div>`
)
...
func ParseProfile(contents []byte) engine.ParseResult {
	...
	//兴趣爱好
	profile.Food = extractString(contents, foodMatch)   //喜欢的食物
	profile.Idol = extractString(contents, idolMatch)   //喜欢的名人
	profile.Song = extractString(contents, songMatch)   //喜欢的歌
	profile.Book = extractString(contents, bookMatch)   //喜欢的书
	profile.Hobby = extractString(contents, thingMatch) //喜欢做的事
	...
	for k, val := range matches {
		switch k {
		case AgeMate:
			age := val[1][0 : len(val[1])-3] //去除末尾中文"岁"
			profile.AgeMate = string(age)
		case Height:
			profile.HeightMate = string(val[1])
		case WorkplaceMate:
			//profile.WorkplaceMate =
		}
	}
